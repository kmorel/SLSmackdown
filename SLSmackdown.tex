...
sort-last parallel rendering library\sticky{cite users' guide?}.  Although
originally created to capture sort-last rendering algorithms for tiled
...
IceT is used in multiple production products like ParaView and
VisIt\sticky{cite?} and has been used to achieve record-breaking rendering
rates\sticky{cite}.  As such, IceT is an excellent code base for creating,
testing, and comparing image compositing algorithms.  It already contains
routines for efficiently capturing, compressing, and compositing images.
It also contains efficient existing algorithms to which to compare new
ones.  Furthermore, any optimizations or new algorithms added to IceT can
be applied to existing production software.
...
\begin{figure}[htb]
  \centering
  \sticky{Output of SimpleTiming.}
  \caption{Example of images rendered in our experiments.}
